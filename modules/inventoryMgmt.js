...
function getProductByKeyWords(string) {
  ...
  return new Promise((resolve, reject) => {
    if (/^[a-zA-Z0-9 ]*$/.test(string) && string.trim().length < 3)
      return reject("Please double check your input 请检查您的输入");
      ...
      .then((product) => {
        ...
        if (!isAlternative) {
          ...
          if (product.alternativeShelfLocation) {
            ...
            return reject(
              "This product already had an alternative shelf location."
            );
          } else {
